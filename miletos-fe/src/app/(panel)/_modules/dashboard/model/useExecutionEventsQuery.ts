...
export function useExecutionEventsQuery(
  ...
  { enabled = true, pollingEnabled = false, companyId }: UseExecutionEventsQueryOptions = {},
) {
  return useQuery<ExecutionEventPageResponse, ApiError>({
    queryKey: executionQueryKeys.events(executionId, params, companyId),

    queryFn: async () => {
      try {
        return await listExecutionEvents(executionId, params, companyId);
      } catch (error) {
