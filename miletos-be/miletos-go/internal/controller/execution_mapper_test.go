...
func TestMapWorkflowRequestPreservesContractData(t *testing.T) {
	request := workflowRequest{
		...
		Nodes: []nodeRequest{{
			...
			Position:      &model.NodePosition{X: 10, Y: 20},
		}},
		Edges: []edgeRequest{{
...
func TestExecutionResponseMappingAndJSONNames(t *testing.T) {
	...
	response := mapExecutionOutcome(engine.ExecutionOutcome{Execution: execution, ScheduledRoots: 2})
...
func TestMapExecutionPage(t *testing.T) {
	...
	if mapped["next"] != "exec-next" || mapped["hasNext"] != true {
		t.Fatalf("mapped page = %#v", mapped)
	}
	items, ok := mapped["items"].([]executionSummaryResponse)
	if !ok || len(items) != 1 || items[0].ExecutionID != "exec-1" {
...
func TestWriteExecutionErrorMappings(t *testing.T) {
	tests := []struct {
		...
		{
			name: "async unavailable", err: engine.ErrAsyncUnavailable,
			status: http.StatusServiceUnavailable, code: "EXECUTION_UNAVAILABLE",
		},
		{
			name: "recovery", err: engine.ErrRecoveryUnsupported,
			status: http.StatusConflict, code: "RECOVERY_NOT_SUPPORTED",
		},
		{
			name: "validation", err: engine.ErrInvalidWorkflow,
...
func TestWriteAPIErrorIncludesRequestContextAndOmitsEmptyDetails(t *testing.T) {
	...
	handler := middleware.RequestContext(next)
	request := httptest.NewRequest(http.MethodGet, "/", nil)
	request.Header.Set(middleware.HeaderRequestID, "req-test")
	request.Header.Set(middleware.HeaderCorrelationID, "corr-test")
	response := httptest.NewRecorder()
