...
func (repository *ExecutionRepository) Create(
	...
	_, err = transaction.Exec(ctx, `
		...
		snapshotID, mode, correlationID, execution.Status, now,
	)
	if err != nil {
...
func (repository *ExecutionRepository) FindByID(
	...
	execution, err := scanExecution(repository.db.QueryRow(ctx, `
		SELECT workflow_execution_id, company_id, workflow_id, workflow_revision,
			snapshot_id, mode, COALESCE(correlation_id, ''), status,
...
func (repository *ExecutionRepository) MarkExecutionQueued(ctx context.Context, executionID string) error {
	now := time.Now().UTC()
	transaction, err := repository.db.Begin(ctx)
	if err != nil {
	...
	command, err := transaction.Exec(ctx, `
		...
		SET status = 'QUEUED', queued_at = COALESCE(queued_at, $2), updated_at = $2,
			lock_version = lock_version + 1
		WHERE workflow_execution_id = $1 AND status = 'CREATED'`,
		executionID, now,
	)
	if err != nil {
		return err
...
func (repository *ExecutionRepository) MarkExecutionRunning(ctx context.Context, executionID string) error {
	now := time.Now().UTC()
	transaction, err := repository.db.Begin(ctx)
	if err != nil {
	...
	command, err := transaction.Exec(ctx, `
		...
		SET status = 'RUNNING', started_at = COALESCE(started_at, $2), updated_at = $2,
			lock_version = lock_version + 1
		WHERE workflow_execution_id = $1 AND status IN ('CREATED', 'QUEUED')`,
		executionID, now,
	)
	if err != nil {
		return err
...
func (repository *ExecutionRepository) Finalize(
	...
	command, err := transaction.Exec(ctx, `
		UPDATE workflow_runtime.workflow_executions
		SET status = $2, terminal_outputs = $3, failure_summary = $4,
			finished_at = $5, updated_at = $5, lock_version = lock_version + 1
		WHERE workflow_execution_id = $1
		  AND status NOT IN ('SUCCEEDED', 'FAILED', 'CANCELLED', 'TIMED_OUT')`,
		executionID, status, encodedOutputs, encodedFailure, now,
	)
	if err != nil {
...
func scanExecution(row rowScanner) (model.Execution, error) {
	...
	err := row.Scan(
		...
		&execution.CorrelationID, &execution.Status, &execution.CreatedAt,
