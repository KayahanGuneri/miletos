...
type DefinitionLookup func(string, string) (model.NodeDefinition, bool)
type TypeLookup func(string) bool
type ConfigurationValidator func(model.Node) error

func ValidateWorkflow(workflow model.Workflow, nodeDefined func(string) bool) error {
	inputPorts := make([]model.Port, 0, len(workflow.Edges))
	outputPorts := make([]model.Port, 0, len(workflow.Edges))
	for _, edge := range workflow.Edges {
		inputPorts = append(inputPorts, model.Port{Name: edge.TargetInputPort})
		outputPorts = append(outputPorts, model.Port{Name: edge.SourceOutputPort})
	}
	lookup := func(nodeType, _ string) (model.NodeDefinition, bool) {
		if nodeDefined == nil || nodeDefined(nodeType) {
			return model.NodeDefinition{
				...
				InputEdgeConstraint: model.EdgeConstraint{Unlimited: true},
				OutputEdgeConstraint: model.EdgeConstraint{Unlimited: true},
			}, true
		}
		return model.NodeDefinition{}, false
	}
...
func ValidateWorkflowDefinition(
	...
	nodes := append([]model.Node(nil), workflow.Nodes...)
	...
	definitions := make(map[string]model.NodeDefinition, len(nodes))
	for _, node := range nodes {
		...
		if validateConfiguration != nil {
			if err := validateConfiguration(node); err != nil {
				issues = append(issues, ValidationIssue{
	...
	edges := append([]model.Edge(nil), workflow.Edges...)
	...
	if cyclePath := FindCyclePath(workflow); len(cyclePath) > 0 {
		issues = append(issues, ValidationIssue{
...
func ValidateNode(node model.Node, nodeDefined func(string) bool) error {
	if strings.TrimSpace(node.ID) == "" {
	...
	if nodeDefined != nil && !nodeDefined(node.Type) {
		return fmt.Errorf("node %q uses undefined type %q", node.ID, node.Type)
	}
	return nil
}

func ValidateExecutionRequest(workflow model.Workflow) error {
	if len(workflow.Nodes) > 1000 {
...
func hasPort(ports []model.Port, requested string) bool {
...
func portNames(ports []model.Port) string {
...
func constraintIssues(
	...
	direction string,
	actual uint,
	constraint model.EdgeConstraint,
) []ValidationIssue {
	...
	if !constraint.Unlimited && constraint.Maximum != nil && actual > *constraint.Maximum {
		issues = append(issues, ValidationIssue{
