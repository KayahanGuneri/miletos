...
func (controller *PublicController) Invoke(
	...
	payload := mapWebhookPayload(request, body, requestID, correlationID)
	...
	writePublicJSON(writer, http.StatusAccepted, mapPublicExecution(outcome))
}
...
func writePublicError(
	...
	writePublicJSON(writer, status, mapPublicError(
		status, code, message,
		requestcontext.RequestID(request.Context()),
