...
func builtinHandler(t *testing.T, name string) plugin.NodeHandler {
	t.Helper()
...
func TestStaticInputNode(t *testing.T) {
	...
	output, err := handler(context.Background(), plugin.NodeExecutionContext{}, map[string]any{"value": "configured"}, nil)
	if err != nil || output != "configured" {
		t.Fatalf("handler returned (%#v, %v)", output, err)
	}
	if _, err := handler(context.Background(), plugin.NodeExecutionContext{}, map[string]any{}, nil); err == nil {
		t.Fatal("missing value error = nil")
	}
	if _, err := handler(context.Background(), plugin.NodeExecutionContext{}, map[string]any{"value": "x"}, "input"); err == nil {
...
func TestPassThroughAndTerminalNodes(t *testing.T) {
	for _, name := range []string{"core.pass-through", "core.terminal"} {
		t.Run(name, func(t *testing.T) {
			...
			output, err := handler(context.Background(), plugin.NodeExecutionContext{}, nil, "payload")
			if err != nil || output != "payload" {
				t.Fatalf("handler returned (%#v, %v)", output, err)
			}
			if _, err := handler(context.Background(), plugin.NodeExecutionContext{}, nil, nil); err == nil {
...
func TestDelayNodeHonorsCancellationAndValidatesConfiguration(t *testing.T) {
	...
	_, err := handler(ctx, plugin.NodeExecutionContext{}, map[string]any{"delay": "1h"}, "payload")
	...
	for _, configuration := range []map[string]any{
		...
		if _, err := handler(context.Background(), plugin.NodeExecutionContext{}, configuration, "payload"); err == nil {
