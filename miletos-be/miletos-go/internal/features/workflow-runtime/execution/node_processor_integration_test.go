...
func TestNodeProcessorIsolatesPanicAndContinuesIndependentBranchIntegration(t *testing.T) {
	...
	initialJobs := dispatchDueNodeCommands(
		t,
		nodeQueue,
		2,
	)
	...
	if attemptStatus != string(model.NodeFailed) || attemptNumber != panicJob.Attempt {
		t.Fatalf("panic attempt = (%s, %d)", attemptStatus, attemptNumber)
	}
}

func TestNodeProcessorRetryableFailureThenExhaustionIntegration(t *testing.T) {
	pool := engineIntegrationDatabase(t)
	workflow := workflowfeature.Workflow{
		ID: "retry-exhaustion", CompanyID: "company-1", Name: "Retry Exhaustion", Revision: 1,
		Nodes: []workflowfeature.WorkflowNode{{ID: "retry", Type: "retry-node", Version: "v1"}},
	}
	execution := seedEngineExecution(t, pool, workflow, "ASYNC")
	executions := repository.NewExecutionRepository(engineDatabaseClient(t))
	nodeQueue := &recordingQueue{}
	...
	if err := processor.Process(context.Background(), firstJob); err != nil {
		t.Fatalf("first retryable Process() error = %v", err)
	}

	jobs := nodeQueue.snapshot()
	if len(jobs) != 1 {
		t.Fatalf("retryable failure unexpectedly pushed directly to the queue: %#v", jobs)
	}
	...
	if err := processor.Process(context.Background(), firstJob); err != nil {
		t.Fatalf("exhausting retryable Process() error = %v", err)
	}
	if calls.Load() != 2 {
		t.Fatalf("retryable handler calls = %d, want 2", calls.Load())
	}
	if jobsAfter := nodeQueue.snapshot(); len(jobsAfter) != 1 {
		t.Fatalf("retry exhaustion queued another job: %#v", jobsAfter)
	}
	failed, err := executions.FindNode(
		context.Background(), workflow.CompanyID, execution.ID, "retry",
	)
	if err != nil || failed.Status != model.NodeFailed || failed.Attempt != 2 {
		t.Fatalf("exhausted retryable node = (%#v, %v)", failed, err)
	}
	completed, err := executions.FindByID(
		context.Background(), workflow.CompanyID, execution.ID,
	)
	if err != nil || completed.Status != model.ExecutionFailed {
		t.Fatalf("exhausted retryable execution = (%#v, %v)", completed, err)
	}
...
func assertSafePanicText(t *testing.T, text string, secrets ...string) {
	t.Helper()
	if !strings.Contains(text, "failed unexpectedly") &&
		!strings.Contains(text, "outcome was persisted") {
		t.Fatalf("panic text = %q, want a safe public failure phrase", text)
	}
...
func TestNodeProcessorExecutesAndContinuesIntegration(t *testing.T) {
	...
	continuedJobs := dispatchDueNodeCommands(
		t,
		nodeQueue,
		1,
	)
	...
	if terminalJob.ExecutionID != execution.ID {
		t.Fatalf(
			"terminal job execution ID = %q, want %q",
			terminalJob.ExecutionID,
			execution.ID,
		)
	}
...
func TestNodeProcessorRetriesThenSucceedsAndIgnoresDuplicateDeliveryIntegration(t *testing.T) {
	...
	if err := registry.DefineNode("test.retry", func(
		...
		if calls.Add(1) == 1 {
			return nil, &plugin.NodeError{
				Category: string(model.FailureCategoryExecution),
				Code:     "TEST_RETRYABLE_FAILURE", Message: "retry me", CanRetry: true,
			}
		}
		return "success", nil
	}); err != nil {
