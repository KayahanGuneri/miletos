...
const (
	...
	defaultCommandTopic          = "miletos.workflow.node.commands.v1"
	defaultDeadLetterTopic       = "miletos.workflow.node.dead-letter.v1"
)
...
func NewKafka(brokers []string, clientID, consumerGroup string) (*Kafka, error) {
	return NewKafkaWithConcurrency(
		brokers, clientID, consumerGroup, 2, defaultCommandTopic, defaultDeadLetterTopic,
	)
}
...
func NewKafkaWithConcurrency(
	...
	for _, broker := range brokers {
		if strings.TrimSpace(broker) == "" {
			return nil, fmt.Errorf("Kafka brokers must not contain empty values")
		}
	}
	...
	if strings.TrimSpace(commandTopic) == "" {
		return nil, fmt.Errorf("Kafka command topic is required")
	}
	if strings.TrimSpace(deadLetterTopic) == "" {
		return nil, fmt.Errorf("Kafka dead-letter topic is required")
	}
