...
func TestExecutionJSONContract(t *testing.T) {
	...
	if document["status"] != string(model.ExecutionSucceeded) {
...
func TestNodeExecutionJSONContract(t *testing.T) {
	...
	if document["status"] != string(model.NodeRetryPending) {
		t.Fatalf("status = %v", document["status"])
	}
}
...
func TestContractStatusStrings(t *testing.T) {
	statuses := map[string]string{
		"VALIDATING":    string(model.ExecutionValidating),
		"REJECTED":      string(model.ExecutionRejected),
		"RUNNING":       string(model.ExecutionRunning),
		"SUCCEEDED":     string(model.ExecutionSucceeded),
		"FAILED":        string(model.ExecutionFailed),
		"CANCELLED":     string(model.ExecutionCancelled),
		"READY":         string(model.NodeReady),
		"RETRY_PENDING": string(model.NodeRetryPending),
		"SKIPPED":       string(model.NodeSkipped),
	}
