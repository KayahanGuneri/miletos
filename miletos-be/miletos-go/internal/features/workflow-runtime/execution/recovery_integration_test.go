...
func TestRecoveryPreservesSuccessfulNodesAndSchedulesResetNodesIntegration(t *testing.T) {
	...
	scheduler := executionfeature.NewScheduler(workflows, executions, nodeQueue, "commands", registry)
	...
	outcome, err := recovery.Recover(
		context.Background(), workflow.CompanyID, source.ID, "recovery-key",
	)
...
func TestRecoveryRejectsMissingAndNonFailedSourcesIntegration(t *testing.T) {
	...
	scheduler := executionfeature.NewScheduler(
		workflows, executions, &recordingQueue{}, "commands", registry,
	)
	recovery := executionfeature.NewRecoveryService(
		workflows, executions, workflowfeature.NewWorkflowService(workflows, registry), scheduler,
	)
	...
	if _, err := recovery.Recover(
		context.Background(), workflow.CompanyID, "missing", "key-1",
	); !errors.Is(err, repository.ErrNotFound) {
		t.Fatalf("missing source error = %v", err)
	}
	if _, err := recovery.Recover(
		context.Background(), workflow.CompanyID, source.ID, "key-2",
