...
func TestExecutionRepositoryIntegration(t *testing.T) {
	...
	if err := repository.MarkExecutionQueued(
		context.Background(), workflow.CompanyID, second.ID,
	); err != nil {
		t.Fatalf("MarkExecutionQueued() error = %v", err)
	}
	if err := repository.MarkExecutionRunning(
		context.Background(), workflow.CompanyID, second.ID,
	); err != nil {
		t.Fatalf("MarkExecutionRunning() error = %v", err)
	}
	if err := repository.Finalize(
		context.Background(), workflow.CompanyID, second.ID, model.ExecutionSucceeded,
...
func TestExecutionRepositoryIdempotencyIntegration(t *testing.T) {
	...
	repository := NewExecutionRepository(client)
	snapshot, err := workflowRepository.Save(context.Background(), workflow)
	if err != nil {
		t.Fatalf("Save() error = %v", err)
	}
	created, err := repository.Create(
		context.Background(), workflow, snapshot.ID, "ASYNC",
		model.ExecutionOriginManualDirect, "corr-1", "key-1", strings.Repeat("a", 64),
	)
	if err != nil {
	...
	found, replayed, err := repository.FindIdempotent(
		context.Background(), workflow.CompanyID, "key-1", strings.Repeat("a", 64),
	)
	...
	if _, _, err := repository.FindIdempotent(
		context.Background(), workflow.CompanyID, "key-1", strings.Repeat("b", 64),
