...
func TestMarkNodeQueuedPersistsOneJSONBValueAndPreservesStateOnFailureIntegration(
	t *testing.T,
) {
	...
	node, changed, err := repository.MarkNodeQueued(
		context.Background(), workflow.CompanyID, execution.ID, "root", input,
	)
	if err != nil || !changed {
	...
	if err := pool.QueryRow(context.Background(), `
		...
		WHERE company_id = $1
		  ...
		  AND node_id = $3`,
		workflow.CompanyID, execution.ID, "root",
	).Scan(&encoded); err != nil {
		t.Fatalf("load input_summary: %v", err)
	}
	var persisted map[string]any
	if err := json.Unmarshal(encoded, &persisted); err != nil {
		t.Fatalf("input_summary is not one JSONB document: %v", err)
	}
	if persisted["method"] != "POST" {
		t.Fatalf("persisted input_summary = %#v", persisted)
	}
	if _, changed, err := repository.MarkNodeQueued(
		context.Background(), workflow.CompanyID, execution.ID, "root",
	); err != nil || changed {
		t.Fatalf("nil replay MarkNodeQueued() changed=%v err=%v", changed, err)
	}
	var replayed []byte
	if err := pool.QueryRow(context.Background(), `
		SELECT input_summary
		...
		t.Fatalf("load replayed input_summary: %v", err)
	}
	if string(replayed) != string(encoded) {
		t.Fatalf("nil replay replaced input_summary: before=%s after=%s", encoded, replayed)
	}

	_, changed, err = repository.MarkNodeQueued(
		context.Background(), workflow.CompanyID, execution.ID, "terminal",
		map[string]any{"invalid": make(chan int)},
	)
	if err == nil || changed {
		t.Fatalf("unencodable MarkNodeQueued() = (changed=%v, err=%v)", changed, err)
	}
	terminal, err := repository.FindNode(
		context.Background(), workflow.CompanyID, execution.ID, "terminal",
	)
	if err != nil || terminal.Status != model.NodePending {
		t.Fatalf("terminal after failed queue transaction = (%#v, %v)", terminal, err)
	}
}
...
func TestNodeRepositorySuccessIntegration(t *testing.T) {
	...
	isolated, err := repository.ListNodes(
		context.Background(), "another-company", execution.ID, "", 100,
	)
	if err != nil || len(isolated.Items) != 0 {
		t.Fatalf("tenant-isolated ListNodes() = (%#v, %v)", isolated, err)
	}
	if _, err := repository.FindNode(
		context.Background(), "another-company", execution.ID, "root",
	); err != ErrNotFound {
		t.Fatalf("tenant-isolated FindNode() error = %v", err)
	}
