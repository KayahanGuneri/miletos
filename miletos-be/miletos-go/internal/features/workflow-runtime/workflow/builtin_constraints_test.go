...
func TestBuiltinJoinAndPassThroughInputConstraints(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "pass-through requires an incoming edge", targetType: "core.pass-through",
			wantCode: "INPUT_EDGE_COUNT_BELOW_MINIMUM",
		},
		{
...
func TestNilMaximumAllowsMultipleOutgoingEdges(t *testing.T) {
	registry := plugin.NewNodeRegistry()
	if err := plugin.RegisterBuiltinNodes(registry); err != nil {
		t.Fatalf("RegisterBuiltinNodes() error = %v", err)
	}
	definition := workflow.Workflow{
		...
		Nodes: []workflow.WorkflowNode{
			...
			{ID: "terminal-1", Type: "core.terminal", Version: "v1"},
			{ID: "terminal-2", Type: "core.terminal", Version: "v1"},
		},
		Edges: []workflow.Edge{
			builtinEdge("edge-1", "root", "terminal-1"),
			builtinEdge("edge-2", "root", "terminal-2"),
		},
	}
