	...
	lookup := func(nodeType, _ string) (plugin.NodeDefinition, bool) {
		if nodeDefined == nil || nodeDefined(nodeType) {
			return plugin.NodeDefinition{
				...
				InputEdgeConstraint:  plugin.EdgeConstraint{Unlimited: true},
				OutputEdgeConstraint: plugin.EdgeConstraint{Unlimited: true},
			}, true
		}
...
func ValidateNode(node WorkflowNode, nodeDefined func(string) bool) error {
	if strings.TrimSpace(node.ID) == "" {
