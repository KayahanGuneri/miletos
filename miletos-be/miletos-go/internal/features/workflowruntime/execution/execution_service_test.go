...
func TestExecutionServiceRejectsInitialVariablesBeforePersistence(t *testing.T) {
	registry := plugin.NewNodeRegistry()
	if err := plugin.RegisterBuiltinNodes(registry); err != nil {
		t.Fatalf("RegisterBuiltinNodes() error = %v", err)
	}
	...
	if !errors.Is(err, execution.ErrInitialVariablesNotAccepted) {
		t.Fatalf("ExecuteSync() error = %v, want ErrInitialVariablesNotAccepted", err)
	}
}
